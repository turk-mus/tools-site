// يعمل على Netlify (Node 18+)
// يتصل بـ RapidAPI (twitter-trends-by-location)
// يجلب locationId للسعودية إن لم يتم تمريره يدوياً
// مع تصفية كلمات مسيئة + كاش 15 دقيقة
...
// قائمة تصفية أولية (وسّعها لاحقًا)
const BAD_PATTERNS = [
...
// جلب قائمة المواقع ثم إيجاد السعودية
async function getSaudiLocationId(rapidKey) {
  ...
  const byCode = list.find(
    x => (x.countryCode || x.country_code || "").toUpperCase() === "SA"
  );
...
async function fetchTrendsByLocationId(rapidKey, locationId) {
  ...
  const items =
    json?.trends || json?.data || json?.topics || json?.[0]?.trends || json || [];
  ...
  const trends = (Array.isArray(items) ? items : [])
    .map(t => {
      ...
      const promoted = !!t.promoted_content || !!t.promoted || !!t.isPromoted;
...
export const handler = async (event) => {
  try {
    ...
    const locationIdParam = qs.get("locationId");
